...
async def handle_deposit(
    amount: int, update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    if replying:
        if deposited:
            ...
            await update.callback_query.edit_message_text(
                f"Failed to deposit {amount}, Insufficient funds"
            )
    else:
        if deposited:
...
async def handle_withdraw(
    ...
    if replying:
        if withdrew:
            ...
            await update.callback_query.edit_message_text(
                f"Failed to withdraw {amount}, Insufficient funds"
            )
    else:
        if withdrew:
...
async def handle_add_method(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.callback_query.edit_message_text(
        "Select payment method type:", reply_markup=InlineKeyboardMarkup(buttons)
    )
    context.user_data["adding_method"] = True


async def handle_method_type_selection(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    query = update.callback_query
    ...
    if method_type == "bank":
        ...
        await query.edit_message_text(
            "Select cryptocurrency:", reply_markup=InlineKeyboardMarkup(buttons)
        )


async def show_method_selection(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    user_id = update.effective_user.id
    ...
    for method in methods:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    label, callback_data=f"{SELECT_METHOD}{method['type']}"
                )
            ]
        )
    ...
    await update.message.reply_text(
        "Select payment method:", reply_markup=InlineKeyboardMarkup(buttons)
    )
